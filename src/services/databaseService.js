...
export const databaseService = {
    ...
    async getIssues(filters = {}) {
        try {
            console.log('🗃️ Database service getIssues called with filters:', filters);
        ...
        `)
            ...
            if (filters.category && filters.category !== 'all' && filters.category !== '') {
                console.log('🏷️ Applying category filter:', filters.category);
            ...
            console.log('📥 Raw database results:', {
                ...
                sampleData: (data && data.slice(0, 2).map(issue => ({
                    id: issue.id,
                    title: issue.title,
                    category: issue.category,
                    status: issue.status,
                    hasLocation: !!(issue.latitude && issue.longitude),
                    location: issue.latitude && issue.longitude ? `${issue.latitude},${issue.longitude}` : null
                }))) || []
            });
            ...
            if (filters.userLocation && filters.distance) {
                ...
                filteredData = data.filter(function(issue) {
                    if (!issue.latitude || !issue.longitude) {
                        console.log('⚠️ Issue missing location:', issue.id, issue.title);
                        return false;
                    }

                    // Handle different coordinate formats
                    const userLat = filters.userLocation.latitude || filters.userLocation.lat;
                    const userLng = filters.userLocation.longitude || filters.userLocation.lng;

                    if (!userLat || !userLng) {
                        console.log('⚠️ User location incomplete:', filters.userLocation);
                        return true; // Include all issues if user location is incomplete
                    }
                    ...
                    if (!withinRange) {
                        console.log('📏 Issue outside range:', {
                            issueId: issue.id,
            ...
            console.log('✅ Final filtered issues:', {
                count: filteredData.length,
                issues: filteredData.slice(0, 3).map(i => ({
                    id: i.id,
                    title: i.title,
                    category: i.category,
                    status: i.status
    ...
    async createIssue(issueData) {
        try {
            ...
            if (issueData.photoFiles && issueData.photoFiles.length > 0) {
                ...
                console.log('Uploading photos to Supabase Storage...', issueData.photoFiles.length);
                ...
                for (let i = 0; i < issueData.photoFiles.length; i++) {
                    ...
                    try {
                        ...
                        // Only add URLs that are actual Storage URLs, not Base64
                        if (photoUrl && (photoUrl.startsWith('http') || photoUrl.startsWith('https'))) {
                            photoUrls.push(photoUrl);
                            console.log('Successfully uploaded photo to Storage:', photoUrl);
                        } else {
                            console.error('Invalid photo URL returned:', photoUrl);
                            throw new Error('Invalid photo URL returned from storage');
                        }
                    } catch (uploadError) {
                        console.error('Failed to upload photo to Supabase Storage:', uploadError);

                        // Don't use Base64 fallback - throw error to inform user
                        throw new Error(`Photo upload failed: ${uploadError.message}. Please check your internet connection and try again.`);
                    }
                }

                console.log('All photos uploaded successfully to Storage. Total:', photoUrls.length);
            }

            // Legacy fallback - only use if no photoFiles but have base64 photos
            else if (issueData.photos && issueData.photos.length > 0) {
                console.warn('Using legacy base64 photo storage (not recommended)');
                photoUrls = issueData.photos.filter(photo =>
                    photo && (photo.startsWith('http') || photo.startsWith('https') || photo.startsWith('data:'))
                );
            }
            ...
            const issueToInsert = {
                ...
                latitude: issueData.location && typeof issueData.location.latitude === 'number' ?
                    Number(issueData.location.latitude) : null,
                longitude: issueData.location && typeof issueData.location.longitude === 'number' ?
                    Number(issueData.location.longitude) : null,
            ...
            console.log('🗺️ Location debugging:', {
                hasLocation: !!issueData.location,
                locationObject: issueData.location,
                extractedLatitude: issueData.location ? issueData.location.latitude : 'N/A',
                extractedLongitude: issueData.location ? issueData.location.longitude : 'N/A',
                latitudeType: issueData.location ? typeof issueData.location.latitude : 'N/A',
                longitudeType: issueData.location ? typeof issueData.location.longitude : 'N/A',
                finalLatitude: issueToInsert.latitude,
                finalLongitude: issueToInsert.longitude
            });
            ...
            if (issueError) {
                console.error('Issue insertion failed:', issueError);

                // Provide more specific error messages
                if (issueError.message.includes('row-level security') || issueError.message.includes('policy')) {
                    throw new Error('Database permission denied. Please check RLS policies or run disable-rls-dev.sql for development.');
                } else if (issueError.message.includes('duplicate')) {
                    throw new Error('Duplicate issue detected. Please try again.');
                } else {
                    throw new Error(`Failed to create issue: ${issueError.message}`);
                }
            }

            console.log('Issue inserted successfully, now creating status history...');

            // Create initial status history entry (make this optional)
            try {
                ...
                if (historyError) {
                    console.warn('Failed to create status history (this is optional):', historyError);
                }
            } catch (historyError) {
                console.warn('Status history creation failed (continuing anyway):', historyError);
            }
    ...
    async getCategoryStatistics() {
        try {
            ...
            if (error) {
                handleSupabaseError(error, 'get category statistics');
            }
    ...
    // Test database connection and insertion (for debugging)
    async testDatabaseConnection() {
        try {
            ...
            console.log('✅ Database connected. Total issues:', count);
            ...
            if (insertError) {
                ...
                if (insertError.message.includes('row-level security')) {
                    console.log('💡 Solution: Run disable-rls-quick.sql in Supabase SQL Editor');
                }

                return { success: false, error: insertError };
            }

            console.log('✅ Test insertion successful:', inserted.id);

            // Clean up
            await supabase.from('civic_issues').delete().eq('id', inserted.id);
    ...
    async getAdminAnalytics() {
        try {
            ...
            categoryData ? categoryData.forEach(issue => {
